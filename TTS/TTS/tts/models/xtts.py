...
# Compatibility patch: define LanguageManager if missing
class LanguageManager:
    def __init__(self, config=None):
        self.language_ids = {"en": 0, "hi": 1}

    def save_ids_to_file(self, output_path):
        """Compatibility patch for Trainer: saves dummy language ID mappings."""
        import json, os
        os.makedirs(output_path, exist_ok=True)
        file_path = os.path.join(output_path, "language_ids.json")
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(self.language_ids, f, ensure_ascii=False, indent=2)
        print(f"✅ Language IDs saved at {file_path}")
...
class Xtts(BaseTTS):
    ...
    def load_checkpoint(
        ...
        if checkpoint_path is not None:
            ...
            raise ValueError("Both checkpoint_path and checkpoint_dir are None. Provide a valid model checkpoint.")
        ...
        if vocab_path is not None and os.path.exists(vocab_path):
            ...
            print("⚠️ No vocab.json found — skipping tokenizer initialization.")
    ...
    def get_criterion(self):
        """Return a default dummy loss for compatibility with Trainer."""
        import torch.nn as nn
        # You can later replace this with the real XTTS training loss
        return nn.MSELoss()
