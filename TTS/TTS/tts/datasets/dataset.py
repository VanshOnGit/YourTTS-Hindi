...
class TTSDataset(Dataset):
    ...
    def _compute_lengths(samples):
        ...
        for item in samples:
            import os
            base_path = "/home/himanshubeniwal/photon_project/vansh/hindi_data/clean"
            audio_path = item["audio_file"]
            if not os.path.isabs(audio_path):
                audio_path = os.path.join(base_path, audio_path.lstrip("/"))
            audio_path = audio_path.replace("wavs/wavs/", "wavs/").replace(".wav.wav", ".wav")
            if not os.path.exists(audio_path):
                print(f"⚠️ Skipping missing file during dataset load: {audio_path}")
                continue
            item["audio_file"] = audio_path
